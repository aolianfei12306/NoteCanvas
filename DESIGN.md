...
- `pages`?????????????
- `activePageId`????????
- ?? `BoardPageRecord` ?? `width`?`height`?`layers`?`activeLayerId`?`textBlocks`?`strokes`?`shapes`?
- `TextBlockRecord`?`StrokeRecord`?`ShapeRecord` ??? `layerId` ?????????

???????????????????? `width/height/textBlocks/strokes` ??????????????
